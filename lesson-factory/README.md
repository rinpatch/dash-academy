...
See [`AUTHORING.md`](AUTHORING.md) for the lesson file contract.

Read the repository [`AGENTS.md`](../AGENTS.md) before operating the factory. Lesson agents are instructed through [`write-dash-lesson`](../.agents/skills/write-dash-lesson/SKILL.md) and must use the vendored [`dash-docs`](../.agents/skills/dash-docs/SKILL.md) sources. The fixed scope, ordering, verification type, and challenge IDs live in [`curriculum.json`](curriculum.json).
...
npm run lessons -- monitor
npm run lessons -- monitor --watch
