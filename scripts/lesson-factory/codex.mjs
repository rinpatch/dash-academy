...
function buildPrompt(role, lesson, context) {
  ...
  if (role === "author") return `${common}\nApproved research and answers:\n${JSON.stringify(context, null, 2)}\nWrite only content/academy/${lesson.slug}.mdx, lesson-sources/${lesson.slug}.json, tests/lessons/${lesson.slug}.mjs, and tests/lessons/${lesson.slug}.verify.mjs. Important: curriculum prerequisites are slugs, but MDX frontmatter prerequisites must be the corresponding numeric module IDs required by source.config.ts. The verifier must use the independent WASM SDK and only public learner output. Do not commit. Return only the schema result.`;
  ...
  return `${common}\nResearch, answers, diff, and tests:\n${JSON.stringify(context, null, 2)}\nReview independently. Do not edit files. Use verdict revise for any issue an author can fix, including schema, build, test, citation, or pedagogy defects. Use block only when a missing human decision or irreconcilable source conflict makes revision impossible. Return only the schema result.`;
}
