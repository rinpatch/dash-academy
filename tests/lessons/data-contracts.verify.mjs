...
if (
  !learnerOutput.documentSchemas
  || typeof learnerOutput.documentSchemas !== "object"
  || Array.isArray(learnerOutput.documentSchemas)
) {
...
try {
  contract = new DataContract({
    ...
    identityNonce: 1n,
    schemas: learnerOutput.documentSchemas,
    fullValidation: true,
  });
  ...
  const indexes = documentTypes
    .flatMap((name) => contract.schemas[name].indices ?? [])
    .map(({ name }) => name)
    .sort();
