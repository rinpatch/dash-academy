...
export async function liveTest({ lesson, worktree, lessonDir }) {
  ...
  const lessonFiles = path.join(worktree, "lesson-factory/lessons", lesson.slug);
  const fixture = path.join(lessonFiles, "fixture.mjs");
  const verifier = path.join(lessonFiles, "verify.mjs");
